...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const formTitle = typeof body?.form_title === "string" ? body.form_title.trim() : "";
    ...
    const verifyUrl =
      `${appBaseUrl.replace(/\/$/, "")}/verify-public-form-email` +
      `?token=${encodeURIComponent(token)}&form_id=${encodeURIComponent(formId)}`;

    const subject = "Verifica tu correo para acceder al formulario";

    const html = `
      <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #111827;">
        <h2>Verificación de correo</h2>
        <p>Hola,</p>
        <p>Has solicitado acceder${formTitle ? ` al formulario <strong>${escapeHtml(formTitle)}</strong>` : " a un formulario"}.</p>
